...
public class Exo2 {
	...
	public static void main(String[] args) {
		...
		SparkConf conf = new SparkConf().setAppName("Exo2");
		...
		// EXERCICE : La distribution de npatterns. Attention : il ne faut pas y inclure les phases idle
		JavaRDD<String> notIdle = distFile.filter(activity -> 
		{
			String[] split = activity.split(";");
			return !split[0].equals("start") && !split[4].equals("0");
		} );
		//On ne garde que les nPattern pour pouvoir opérer la distribution dessus.
		JavaDoubleRDD nPatternNotIdle = notIdle.mapToDouble(activity -> Double.parseDouble(activity.split(";")[4]));
		...
		synthetyzeToFile(context, "Exo2/distriNpattern.txt", nPatternNotIdleStat, nPatternNotIdlePercentiles, nPatternNotIdleHistogram, nTranches);

		//Partie affichage
		System.out.println("######  EXO 2  ######");
		System.out.println("La distribution de npatterns");
